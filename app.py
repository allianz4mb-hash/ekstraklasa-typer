...
# Ustawienie strony na samym początku
...
if st.session_state.nick == '':
    ...
    # Tworzymy 3 zakładki zawsze, żeby układ się nie zmieniał
    tab_typer, tab_ranking, tab_admin = st.tabs(["🎯 Typer", "🏆 Ranking", "⚙️ Panel Admina"])
    ...
    with tab_admin:
        if st.session_state.nick in ADMINI:
            # Izolujemy panel admina w kontenerze, żeby nie "wyciekał"
            with st.container():
                ...
                if mecze_do_rozliczenia:
                    ...
                    wybrany_str = st.selectbox("Wybierz mecz do rozliczenia:", list(opcje.keys()), key="admin_select")
            ...
            st.warning("Nie masz uprawnień administratora.")
